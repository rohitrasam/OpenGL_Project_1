...
#define SHADER_PATH "D:/Opengl_C/Project_1/res/shaders/003exercise4.shader"
...
void processInput(GLFWwindow *window, float *bias){
...
int main(){
    ...
    while(!glfwWindowShouldClose(window)){
        ...
        processInput(window, &bias);

        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, texture1);
        glActiveTexture(GL_TEXTURE1);
        glBindTexture(GL_TEXTURE_2D, texture2);
        int location = glGetUniformLocation(program, "bias");
        glUniform1f(location, bias);
        glUseProgram(program);
        glBindVertexArray(vao);
